...
function thong_ke(){
    $sql="SELECT tt.ten_trangthai AS TrangThai, SUM(dh.tong_thanhtoan) AS DoanhThu
    FROM trangthai tt
    LEFT JOIN donhang dh ON tt.trangthai_id = dh.trang_thai
    WHERE tt.trangthai_id IN ('1','2','3')
    GROUP BY tt.trangthai_id
    ";
