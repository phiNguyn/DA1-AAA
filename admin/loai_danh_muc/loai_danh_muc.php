...
$hinh='../'.PATH_IMG;
    ...
    foreach($loai_dm as $item){
        extract($item);
        $check_home = ($home == 1) ? 'Có' : 'Không';

        $html_admin.='
        <tr class="text-gray-700">
                                    <td class="h-auto w-[200px] px-4 py-3 font-semibold text-sm border "><a href="index.php?page=sanpham&id_LoaiDanhMuc='.$id.'">'.$name_LDM.'</a></td>
                                    <td class="px-4 py-3 text-md font-semibold border"><img class="w-20" src="'.$hinh.''.$img.'" alt=""></td>
                                    ...
                                    <td class="px-4 py-3 text-lg border">
                                        <a href="index.php?page=update-ldm&id='.$id.'" class="px-2 py-1 font-semibold leading-tight text-blue-700 bg-blue-50 rounded-sm"> Sửa </a>
                                        <a href="index.php?page=delete_ldm&id='.$id.'" class="ml-2 px-2 py-1 font-semibold leading-tight text-red-700 bg-red-50 rounded-sm"> Xóa </a>
        ...
        <div class="flex w-full px-5 justify-between">
            <button
                class="w-fit px-4 py-2 rounded-lg bg-black text-white font-bold text-xl duration-300 hover:shadow-mysd hover:shadow-gray-500 hover:-translate-y-1 hover:-translate-x-1 active:shadow-none active:translate-y-0 active:translate-x-0"><a
                    href="index.php?page=ldm_add">Thêm
                  loại  danh mục</a></button>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr
                                ...
                                <th class="px-4 py-3">Tên loại danh mục</th>
